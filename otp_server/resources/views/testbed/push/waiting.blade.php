...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .card.rejected { border-color: #dc2626; }
        ...
        .result-icon.approved { display: flex; background: #052e16; }
        .result-icon.rejected { display: flex; background: #450a0a; }
...
<div class="card" id="card">
    ...
    <div class="result-icon approved" id="icon-approved" style="display:none">✓</div>
    <div class="result-icon rejected" id="icon-rejected" style="display:none">✕</div>

    <div class="badge"><div class="dot"></div> Awaiting Push Approval</div>
...
const REVERB_APP_KEY = @json($reverbAppKey);
const CHALLENGE_ID   = @json($challengeId);
const EXPIRES_AT     = new Date(@json($expiresAt));
const POLL_URL       = @json(route('testbed.push.poll'));
const HUB_URL        = @json(route('testbed.hub'));
const LOGIN_URL      = @json(route('testbed.push.login'));
...
function handleDecision(decision) {
    ...
    if (decision === 'approved') {
        ...
        setTimeout(() => { window.location.href = HUB_URL + '?success=1'; }, 1800);
...
function startPolling() {
    ...
    pollHandle = setInterval(async () => {
        if (resolved) { clearInterval(pollHandle); return; }
        try {
